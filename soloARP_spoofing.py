...
# Función para obtener la puerta de enlace automáticamente en Windows
def obtener_puerta_enlace():
    # Ejecutar el comando netstat -rn en Windows
    resultado = subprocess.run(["netstat", "-rn"], capture_output=True, text=True)
    lineas = resultado.stdout.splitlines()
    
    # Buscar la línea que contiene la puerta de enlace
    for linea in lineas:
        if "0.0.0.0" in linea:
            partes = linea.split()
            if len(partes) >= 3:
